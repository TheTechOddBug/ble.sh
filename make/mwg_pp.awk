...
function ev1scan_scan(expression, words, _wlen, _i, _len, _c, _t, _w) {
  ...
  for (_i = 0; _i < _len; _i++) {
    ...
    } else if (_c in ev_db_operator) {
      # Note: In the above line, we have been testing the existence of the
      # entry in "ev_db_operator" by «ev_db_operator[_c] != ""».  However,
      # there has been a critical bug in gawk-5.4.1 built without mpfr.  When
      # one attempts to read an unset element of an array, the element is
      # initialized to be 0 unexpectedly.  Gawk is usually built with the mpfr
      # support turned on in distributions, so one might think the impact would
      # limited.  However, MacPorts has shipped gawk 5.4.1 without the mpfr
      # library by default, which means that gawk-5.4.1 fails in macOS.
      #
      # Potentially all the array accesses are affected, where an array may be
      # unset, and where the code distinguishes the array element being 0 and
      # the array element being unset.  It is hard to examine all the
      # occurrences of the array subscripts in the codebase.  We give up it,
      # and we instead introduce fixes to the obvious breakage in the build
      # process of ble.sh.  As a workaround, we use the "in" operator here.
      # With the changes in this branch, at least, ble.sh seems to build
      # produce the identical result compared to the one generated by
      # gawk-5.4.0.  However, it does not ensure that mwg_pp.awk would not be
      # broken in every possible case.
      #
      # https://github.com/akinomyoga/ble.sh/issues/711
